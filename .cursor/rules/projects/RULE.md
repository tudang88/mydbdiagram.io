...
- `src/`: All source code
  - `src/client/` or `src/frontend/`: Frontend code (UI, components)
  - `src/server/` or `src/backend/`: Backend code (API, routes, services)
...
- Frontend: React, Vue, or vanilla TypeScript (to be decided)
...
- **MANDATORY: After completing each phase in IMPLEMENTATION_CHECKLIST.md, perform comprehensive testing**
- **Testing Checklist:**
...
#### Frontend
- `src/client/core/` or `src/frontend/core/`: Core business logic (parsers, validators, diagram engine)
- `src/client/types/` or `src/frontend/types/`: TypeScript type definitions
- `src/client/utils/` or `src/frontend/utils/`: Utility functions
- `src/client/components/` or `src/frontend/components/`: UI components
- `src/client/exporters/` or `src/frontend/exporters/`: Export functionality for different formats
- `src/client/parsers/` or `src/frontend/parsers/`: Input parsers (SQL, JSON, etc.)

#### Backend
- `src/server/routes/` or `src/backend/routes/`: API route handlers
- `src/server/services/` or `src/backend/services/`: Business logic services
- `src/server/middleware/` or `src/backend/middleware/`: Express middleware
- `src/server/types/` or `src/backend/types/`: Backend type definitions
- `src/server/utils/` or `src/backend/utils/`: Backend utility functions
- `src/server/db/` or `src/backend/db/`: Database access layer (if using database)
...
7. **CRITICAL: Write and run comprehensive tests before marking feature complete**
8. Update code documentation if needed
...
- **CRITICAL: After completing each phase/feature, ensure comprehensive testing is performed**
- **Testing Requirements:**
  - Write test scripts or unit tests for new functionality
  - Test all code paths (success and error cases)
  - Verify integration between components
  - Run type checking (`npm run type-check`)
  - Test manually if automated tests are not available
